...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($select_users) > 0) {
        ...
        if ($pass != $cpass) {
            ...
            $stmt = mysqli_prepare($conn, "INSERT INTO `users` (name, birthplace, email, no_hp, address, password, question_forgot, answer_forgot, user_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)");
            if ($stmt) {
                $user_type = 'user';
                mysqli_stmt_bind_param($stmt, "sssssssss", $name, $birthplace, $email, $no_hp, $address, $pass, $question, $answer, $user_type);
